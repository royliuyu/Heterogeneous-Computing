...
int main(int argc, char** argv) {
	...
	unsigned char* output = (unsigned char*)malloc(outputSize * BATCH * sizeof(unsigned char)); //output after convolution with GEMM_RGB image
	...
	char *KernelSource;
	long lFileSize;
	//lFileSize = LoadOpenCLKernel("naiveconv_globalMM_kernel.cl", &KernelSource); //for convolution global memory
	lFileSize = LoadOpenCLKernel("batchconv_kernel.cl", &KernelSource); //for convolution local memory
	...
	while (imagecount<120) {
		...
		for (i = 0; i < BATCH; i++)
		{
			...
			for (j = 0; j < imageGemmRgbSize; j++)
			{
				imageGemmBatchRgb[i*imageGemmRgbSize+j] = imageGemmRgb[j]; //add GEMMed image batch on
			}
		}
		...
		d_image = clCreateBuffer(context, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR, imageGemmBatchRgbSize * sizeof(unsigned char) , imageGemmBatchRgb, &err);
		d_output = clCreateBuffer(context, CL_MEM_WRITE_ONLY, outputSize * BATCH, NULL, &err);
		d_filter = clCreateBuffer(context, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR, FLTSIZE*FLTSIZE * sizeof(float), filter, &err);////
		...
		err |= clEnqueueWriteBuffer(commands, d_filter, CL_TRUE, 0, FLTSIZE*FLTSIZE * sizeof(float), filter, 0, NULL, NULL);////
		...
		err = clEnqueueReadBuffer(commands, d_output, CL_TRUE, 0, outputSize * BATCH * sizeof(unsigned char), output, 0, NULL, NULL);
		if (err != CL_SUCCESS)
		{
	...
	printf("Image amount:%d;  Image size:%d x %d;  Padding:%d;  Stride:%d;  Filter Size:%d; Batch Size:%d .\n", imagecount, WIDTH, HEIGHT, PADDING, STRIDE, FLTSIZE, BATCH);
