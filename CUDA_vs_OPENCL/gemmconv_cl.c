...
int main(int argc, char** argv) {
	...
	unsigned char* output = (unsigned char*)malloc(outputSize * sizeof(unsigned char)); //output after convolution with GEMM_RGB image
	...
	lFileSize = LoadOpenCLKernel("gemmconv_localMM_kernel.cl", &KernelSource); //for convolution local memory
	...
	while (imagecount<120) {
		...
		d_output = clCreateBuffer(context, CL_MEM_WRITE_ONLY, outputSize, NULL, &err);
		...
		err = clEnqueueReadBuffer(commands, d_output, CL_TRUE, 0, outputSize * sizeof(unsigned char), output, 0, NULL, NULL);
		if (err != CL_SUCCESS)
		{
