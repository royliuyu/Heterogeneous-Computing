...
int main(int argc, char** argv) {
	...
	while (imagecount<120) {
		...
		d_image = clCreateBuffer(context, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR, imageGemmRgbSize * sizeof(unsigned char) , imageGemmRgb, &err);
		d_output = clCreateBuffer(context, CL_MEM_WRITE_ONLY, outputSize * CHANNEL, NULL, &err); // 96 channels
		...
		err = clEnqueueWriteBuffer(commands, d_image, CL_TRUE, 0, imageGemmRgbSize, imageGemmRgb, 0, NULL, NULL);
		...
		err |= clSetKernelArg(kernel, 3, sizeof(int), (void *)&imageGemmRgbSize);
		...
		err = clEnqueueReadBuffer(commands, d_output, CL_TRUE, 0, outputSize * CHANNEL * sizeof(unsigned char), output, 0, NULL, NULL);
		if (err != CL_SUCCESS)
		{
