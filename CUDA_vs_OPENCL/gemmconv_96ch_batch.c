...
int main(int argc, char** argv) {
	...
	int imageGemmBatchRgbSize = imageGemmRgbSize * BATCH;
	int outputSize = convHeight * FLTSIZE * FLTSIZE * BATCH * 3;
	...
	unsigned char* imageGemmBatchRgb = (unsigned char*)malloc(imageGemmBatchRgbSize * sizeof(unsigned char));
	unsigned char* output = (unsigned char*)malloc(outputSize * CHANNEL * sizeof(unsigned char)); //output after convolution with GEMM_RGB image, 96 channels
	...
	char *KernelSource;
	long lFileSize;
	//lFileSize = LoadOpenCLKernel("gemmconv_globalMM_kernel.cl", &KernelSource); //for convolution global memory
	lFileSize = LoadOpenCLKernel("gemmconv_localMM_96ch_kernel.cl", &KernelSource); //for convolution local memory
	...
	while (imagecount<120) {
		...
		for (i = 0; i < BATCH; i++)
		{
			...
			for (j = 0; j < imageGemmBatchRgbSize; j++)
			{
				imageGemmBatchRgb[i*imageGemmBatchRgbSize] = imageGemmRgb[i]; //add GEMMed image batch on
			}
		}
		...
		d_image = clCreateBuffer(context, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR, imageGemmBatchRgbSize * BATCH * sizeof(unsigned char) , imageGemmRgb, &err);
		d_output = clCreateBuffer(context, CL_MEM_WRITE_ONLY, outputSize * CHANNEL * BATCH, NULL, &err); // 96 channels
		d_filter = clCreateBuffer(context, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR, FLTSIZE*FLTSIZE * CHANNEL * sizeof(float), filter, &err);// 96 channels
		...
		err = clEnqueueWriteBuffer(commands, d_image, CL_TRUE, 0, imageGemmBatchRgbSize, imageGemmBatchRgb, 0, NULL, NULL);
		err |= clEnqueueWriteBuffer(commands, d_filter, CL_TRUE, 0, FLTSIZE*FLTSIZE * CHANNEL * sizeof(float), filter, 0, NULL, NULL);// 96 channels
		...
		err |= clSetKernelArg(kernel, 3, sizeof(int), (void *)&imageGemmBatchRgbSize);
		err |= clSetKernelArg(kernel, 4, sizeof(int), (void *)&filterSize);
		err |= clSetKernelArg(kernel, 5, sizeof(int), (void *)&channel);
		...
		localWorkSize[0] = 16;
		localWorkSize[1] = 16;
		globalWorkSize[0] = 12*32; //
		globalWorkSize[1] = 4*32; //
		...
		err = clEnqueueReadBuffer(commands, d_output, CL_TRUE, 0, outputSize * CHANNEL * BATCH * sizeof(unsigned char), output, 0, NULL, NULL);
		if (err != CL_SUCCESS)
		{
