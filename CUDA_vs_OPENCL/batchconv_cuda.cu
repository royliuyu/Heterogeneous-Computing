...
int main(void)
{
	...
	cudaMalloc((void**)&image_d, imageGemmBatchRgbSize * sizeof(unsigned char));
	...
	while (imagecount < 120) 
	{
		...
		cudaMemcpy(image_d, imageGemmBatchRgb, imageGemmBatchRgbSize, cudaMemcpyHostToDevice);

		dim3 dimBlock(BLOCKSIZE,BLOCKSIZE);
		dim3 dimGrid((imageGemmBatchRgbSize + BLOCKSIZE - 1) / BLOCKSIZE,(imageGemmBatchRgbSize + BLOCKSIZE - 1) / BLOCKSIZE);

		cudaEventRecord(start, 0);
		convolution <<<dimGrid, dimBlock >>> (image_d, output_d, filter, imageGemmBatchRgbSize, filterSize);//Block-thread
